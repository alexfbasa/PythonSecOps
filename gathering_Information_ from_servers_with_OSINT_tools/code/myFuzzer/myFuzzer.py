...
def main():
    ...
    pars.add_option('-w', '--wordlist', action="store", type="string", dest="wordlist", help="Custom wordlist",
                    default=None)
...
def fuzz(url, CustomWordlist):
    ...
    try:
        ...
        for paths in words:
            ...
            print("[+] [{time}] - [{code}] - [{paths}] -> {fullPath}".format(time=time.strftime("%H:%M:%S"), code=code,
                                                                             paths=paths, fullPath=fullPath))
            if code == "200":
